---
aliases:
  - PG Vector Store Integration - Langchain
Source 1: https://docs.langchain.com/oss/javascript/integrations/vectorstores/pgvector
---
...
* **Key Points:**
  ...
  - To enable vector search in generic PostgreSQL databases, LangChain.js supports using the pgvector Postgres extension.
  - This guide provides a quick overview for getting started with PGVector vector stores.
* **Technical Entities (Classes/Functions/APIs):** `pgvector`, `PostgreSQL`, `LangChain.js`, `PGVectorStore`

## PGVectorStore vs PGVector: Which one to use?
* **Key Points:**
  - LangChain has two Postgres vector store integrations.
* **Technical Entities (Classes/Functions/APIs):** `PGVector (Legacy)`, `PGVectorStore (Modern)`, `langchain-community`, `langchain-postgres (Python)`, `@langchain/pgvector (JS)`, `PGEngine`, `Hybrid Search (Vector + BM25)`
* **Code Snippet:**
```
...
```
* **Key Points:**
  - Use PGVectorStore (Modern—recommended) when: Starting any new project; You need Hybrid Search (vector + keyword combined); You want better performance via connection pooling; You are building async applications (e.g., FastAPI, async Node.js)
  - Use PGVector (Legacy) only when: Maintaining an existing codebase that already uses it; Following an older tutorial that has not been updated yet
  - For all new projects, we recommend using PGVectorStore from langchain-postgres (Python) or @langchain/community (JavaScript).

## Overview
### Integration details
* **Technical Entities (Classes/Functions/APIs):** `PGVectorStore`, `@langchain/pgvector`

## Setup
* **Key Points:**
  - To use PGVector vector stores, set up a Postgres instance with the pgvector extension enabled, then install @langchain/pgvector, the pg driver, and @langchain/core.
  - This guide uses OpenAI embeddings as an example.
  - You can use other supported embeddings models instead.
* **Technical Entities (Classes/Functions/APIs):** `@langchain/pgvector`, `pg`, `@langchain/core`, `@langchain/openai`
* **Code Snippet:**
```bash
...
* **Key Points:**
  - There are many ways to connect to Postgres depending on how you've set up your instance.
  - Here's one example of a local setup using a prebuilt Docker image provided by the pgvector team.
* **Technical Entities (Classes/Functions/APIs):** `pgvector/pgvector:pg16`, `Docker`
* **Code Snippet:**
```yaml
...
### Credentials
* **Key Points:**
  - To connect to your Postgres instance, you'll need corresponding credentials.
  - For a full list of supported options, see the node-postgres docs.
  - If you are using OpenAI embeddings for this guide, you'll need to set your OpenAI key as well
* **Technical Entities (Classes/Functions/APIs):** `node-postgres`, `process.env.OPENAI_API_KEY`, `LangSmith`
* **Code Snippet:**
```typescript
...
* **Key Points:**
  ...
  - This will automatically check for the presence of a table, given by tableName in the passed config. If it is not there, it will create it with the required columns.
...
* **Technical Entities (Classes/Functions/APIs):** `PGVectorStore.initialize()`, `OpenAIEmbeddings`, `PoolConfig`, `DistanceStrategy`, `cosine`, `innerProduct`, `euclidean`
* **Code Snippet:**
```typescript
...
* **Technical Entities (Classes/Functions/APIs):** `Document`, `addDocuments()`, `crypto.randomUUID()`
* **Code Snippet:**
```typescript
...
* **Technical Entities (Classes/Functions/APIs):** `similaritySearch()`, `similaritySearchWithScore()`, `in`, `notIn`, `arrayContains`
* **Code Snippet:**
```typescript
...
* **Technical Entities (Classes/Functions/APIs):** `asRetriever()`, `invoke()`
* **Code Snippet:**
```typescript
...
* **Key Points:**
  - For guides on how to use this vector store for retrieval-augmented generation (RAG), see the following sections: Build a RAG app with LangChain.; Agentic RAG; Retrieval docs
...
* **Technical Entities (Classes/Functions/APIs):** `pg.Pool`, `new PGVectorStore()`, `addDocuments()`, `similaritySearch()`, `end()`
* **Code Snippet:**
```typescript
import { OpenAIEmbeddings } from "@langchain/openai";
import { PGVectorStore } from "@langchain/pgvector";
...
* **Technical Entities (Classes/Functions/APIs):** `HNSW index`, `approximate nearest neighbor (ANN)`, `createHnswIndex()`, `similaritySearchVectorWithScore()`, `end()`
* **Code Snippet:**
```typescript
import { OpenAIEmbeddings } from "@langchain/openai";
import { PGVectorStore } from "@langchain/pgvector";
...
* **Technical Entities (Classes/Functions/APIs):** `end()`
* **Code Snippet:**
```typescript
