...
aliases:
  - NgRx Reducers
Source 1: https://ngrx.io/guide/store/reducers
---
# Reducers
...
* **Technical Entities (Classes/Functions/APIs):** `Action`, `createReducer`, `on`, `createAction`
...
* **Technical Entities (Classes/Functions/APIs):** `State`
* **Code Snippet:**
```typescript
...
* **Technical Entities (Classes/Functions/APIs):** `createReducer`, `on`, `lodash`, `immer`
* **Code Snippet:**
```typescript
...
* **Technical Entities (Classes/Functions/APIs):** `StoreModule.forRoot()`
* **Code Snippet:**
```typescript
...
* **Technical Entities (Classes/Functions/APIs):** `bootstrapApplication`, `provideStore`, `provideState`
* **Code Snippet:**
```typescript
...
* **Technical Entities (Classes/Functions/APIs):** `StoreModule.forFeature()`, `provideState`, `scoreboardFeatureKey`, `createFeatureSelector`
...
* **Key Points:**
  ...
  - Note: Similarly, if you are using effects, you will need to register both EffectsModule.forRoot([...]) and provideEffects([...]).
* **Technical Entities (Classes/Functions/APIs):** `StoreModule.forRoot`, `provideStore`, `EffectsModule.forRoot`, `provideEffects`

## Next Steps
* **Key Points:**
  - Reducers are only responsible for deciding which state transitions need to occur for a given action. In an application there is also a need to handle impure actions, such as AJAX requests, in NgRx we call them Effects.