...
- [Temperature & Sampling:](temperature.md)  Controls output randomness; low temperature deterministic tasks (code,extraction), high for creative tasks.
- [Top-P/Top-K Sampling:](temperature.md) Nuclues sampling strategies that constrin token selection space; tune for quality vs diversity trade-off
- [Hallucination:](hallucination.md)  Model generates plausible but factually incorrect output; a structural risk, not a bug - architecture must compensate via grounding and verification
- [Emerging Capabilities:](emergent-capabilities.md)  Larger models exhibit skills not explicity trained for (reasoning,code gen); capability thresholds vary by model size & training date.
- Multi modal Models: Models accepting text + image/audio/video(GPT-4o, Gemini); expanding input modalities changes integration architecture significantly.
	
## Model Types & Selection

- [Frontier Models (Closed-Source):](frontier-models.md) GPT-4o, Claude 3.5, Gemini 2.5 - highest capability, API -only , vendor lock-in-risk, per-token pricing at scale becomes expensive.
- [Open-Weight Models:](open-weight-models.md) Llama 3.x, Mistral, Qwen, Gemma - self-hostable , no data-egress concerns, ideal for regulated industries and cost optimization at volume
- [Small Language Models(SLMs):](small-lang-models.md) Phi-3/4, Llama 3B/8B - designed to run on device or constrained infra; suitable for narrow, well-defined tasks.
- [Embedding Models:](embedding-models.md) text-embedding-3-large(OpenAI), Cohere Embed, BGE - purpose-built for semantic similarity;separate from generative models, must be versioned independently.
...
1. [System Prompt:](system-prompt.md)  Defines model persona, constraints , output format -acts as the "contract" between the application & the LLM;version-controlled like code.
2. [Few-Shot prompting:](few-shot-prompting.md) Prioviding input-output examples inside the prompt to guide model behaviour without retraining; most cost-effective capability shaping technique.
3. [Chain-of-Thoght(CoT):](cot.md)  Instructing model to reason step by step before answering improves accuracy on multi step tasks, increases token usage & latency.
4. [Structured Output/JSON Mode:](structured-output.md)  Forcing model to respond in a strict schema(JSON,XML) using constrained decoding or output parsers; critical for downstream system integration.
5. [Prompt Injection:](prompt-injection.md) Attack vector where malicious user input overrides system instructions; a security architecture concern not just a prompt concern.
6. [Prompt Versioning & Management:](prompt-versioning.md) Treat prompts a first class artifacts - store in version control (LangSmith, PromptLayer) test regression on prompt changes.
7. [Meta-Prompting:](meta-prompting.md) Using an LLM to generate or optimize prompts for another LLM;emerging pattern for automated prompt tuning in production pipeline

## Fine-Tuning & Model Adaptation

1. [When to fine tune:](fine-tuning.md) Only when propmt engineering + RAG cannot achieve required behaviour, style or domain accuracy; not a first resort
2. [Supervised Fine-Tuning(SFT):](supervised-fine-tuning.md) Training on curated  input-output pairs to adapt model to specific tasks/tone; requires quality data curation, which is the hardest part.
3. [LoRA/QLoRA:](lora-qlora.md) Parameter-efficient fine tuning techniques that add small adapter metrics, reducing GPU memory 4-8x; standard approach for adapting open-weight models.
4. [RLHF(Reinforcement Learning from Human Feedback):](rlhf.md) Alignment technique used during base model training; not typically done by product teams but important to understand its role in model behaviour.
...
6. [DPO(Direct Preference Optimization):](dpo.md) Simpler alternative to RLHF for alignment; trains directly on preference pairs without a reward model- lower infrastructure complexity
...
9. [Catastrophic Forgetting:](catastrphic-forgetting.md) Fine-tuning can degrade model's general capabilities; mitigated by mixing general data with task-specific data.
10. [Model Merging:](model-merging.md) Combining weights from multiple fine-tuned models(TIES, DARE merge); emerging technique to gain multi-task capability without retraining

## LLM Evaluation & Observability

1. [Evals(LLM Evaluation):](evaluation.md)  Systematic testing of LLM outputs against quality criteria - the engineering equivalent of unit/integration tests; non-negotiable in production systems.
...
3. [Reference based vs Reference free Evals:](evaluation-metrics.md) Comparing to ground -truth answers vs assessing quality without a reference; the latter required when no golden dataset exists.
4. [RAGAS/TruLens:](ragas-trulens.md) Eval frameworks specifically for LLM pipelines; measure faithfulness, answer relevance, context precision without manual labeling.
5. [Tracing & Observability:](observability.md) Capturing full LLM call chains(inputs,outputs,latency,token counts,tool calls) using LangSmith, Langfuse or Branitrust for debugging & cost attribution.
...
1. [LLM Gateway/Proxy:](gateway-proxy.md) Centralized Layer(LiteLLM, KongAI , Azure APIM) for routing, rate-limiting, key management, model fallback & cost tracking across all LLM calls.
2. [Semantic Caching:](semantic-caching.md) Caching LLM responses based on semantic similarity of queries (not exact match) using vector lookup; reduce latency & API cost significantly.
3. [Orchestration Layer:](orchestration.md) Coordinates multi-step LLM workflows - LangChain,LlamaIndex or custom code; avoid over relying on framework abstractions in complex systems.
4. [Event Driven LLM pipelines:](event-driven-llm.md) Async processing of LLM tasks via message queues (Kafka,SQL) decoupling inference from request lifecycle for long-running tasks.
5. [Streaming Responses:](streaming-response.md) Token by Token streaming via SSE or WebSockets; critical for perceived UX performance in chat interfaces.
...
8. [Context Management:](context-management.md)  Explicity managing what goes into each LLM call's context window - trimming, summarizing or prioritizing by relevance to stay withing limits
...
## Agentic Patterns without full agents (Workflows)
